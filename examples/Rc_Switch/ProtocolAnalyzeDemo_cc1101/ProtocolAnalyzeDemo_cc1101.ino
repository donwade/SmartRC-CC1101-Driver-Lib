    ...
    if (mySwitch.available())
    {
        ...
        :
            ...
            for (int i = 0; i < 2 * mySwitch.getReceivedBitlength() + 2 - 1 + databitsoffset; i++)
            {
                ...
                if ((i - databitsoffset) % 16 == 0)
                    Serial.println("");
            }

            if ((2 * mySwitch.getReceivedBitlength() + 2 - 1 + databitsoffset - 1) % 16 != 0)
                Serial.println("");

            if (databitsoffset != 1)
                Serial.println(databuffer[2 * mySwitch.getReceivedBitlength() + 1]);
